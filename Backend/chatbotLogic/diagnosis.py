...
def diagnosis(user_id):
    ...
    portfolio = pd.read_sql_query( 
        ...
        cursor, params=(user_id,)
    )
    stock_info = pd.read_sql_query(
        "SELECT ticker, sector, asset_type FROM stock_info", cursor
    )
    cursor.close()
    ...
    portfolio["current_price"] = portfolio.apply(
        ...
        if x["asset_type"] in ["국내주식", "해외주식", "가상자산", "외화"] else x["purchase_price"],
        axis=1
    )
    ...
    portfolio["return"] = portfolio.apply(
        lambda x: 0 if x["asset_type"] == "예적금" else (x["current_price"] - x["purchase_price"]) / x["purchase_price"],
        axis=1
    )
    total_return = (portfolio["weight"] * portfolio["return"]).sum() * 100
    start_date = (datetime.now() - timedelta(days=365)).strftime("%Y-%m-%d")
    portfolio_returns = []
    for _, row in portfolio.iterrows():
        if row["asset_type"] == "예적금":
            portfolio_returns.append(pd.Series(0.0, index=pd.date_range(start_date, "2025-09-22")))
        else:
            prices = fdr.DataReader(row["assetId"] + ("/KRW" if row["asset_type"] in ["가상자산", "외화"] else ""),
                                    start_date)["Close"]
            returns = prices.pct_change().dropna()
            portfolio_returns.append(returns * row["weight"])
    portfolio_std = np.std(sum(portfolio_returns)) * 100
    risk_free_rate = 0.03
    sharpe_ratio = (total_return / 100 - risk_free_rate) / portfolio_std if portfolio_std else 0

    # 기술적 분석
    technicals = [calculate_technical_analysis(row["assetId"], row["asset_type"]) for _, row in portfolio.iterrows()]
    ...
    for _, row in portfolio.iterrows():
        if row["asset_type"] == "예적금":
            values = pd.Series(row["quantity"] * row["purchase_price"], index=pd.date_range(start_date, "2025-09-22"))
        else:
            values = fdr.DataReader(row["assetId"] + ("/KRW" if row["asset_type"] in ["가상자산", "외화"] else ""),
